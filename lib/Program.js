...
(function() {
  var Base, Program, Ui, fs, program,
  ...
  Program = (function(_super) {
    ...
    Program.prototype.initCommander = function() {
      ...
      return program.version(Program.getVersion()).usage('[options] hostname/ip[:port]').option('-v, --verbose', 'verbose').option('-s, --silent', 'do not send message on connection').option('-a, --agent <agent>', 'user agent');
    };
    ...
    Program.prototype.setupHandlers = function() {
      return this.ui.on('input', this.onInput);
    };
    ...
    Program.prototype.charMap = {};

    Program.prototype.onInput = function(c) {
      if (c === 'q') {
        return this.close();
      }
      if (this.charMap[c] != null) {
        return this.log('input received!!', c);
      } else {
        return this.log('unhandled input', c);
      }
    };

...
(function() {
  ...
  Program = (function(_super) {
    ...
    Program.prototype.initCommander = function() {
      ...
      return program.version(Program.getVersion()).usage('[options] hostname/ip[:port]').option('-v, --verbose', 'verbose').option('-d, --debug', 'debug').option('-c, --config <file>', 'config file', process.env[process.platform === 'win32' ? 'USERPROFILE' : 'HOME'] + '/.xbmc-remote-keyboard.conf').option('-u, --username <username>', 'username').option('-P, --password <password>', 'password').option('-s, --host <host>', 'hostname/ip').option('-p, --port <port>', 'port', 9090).option('-S, --silent', 'do not send message').option('-a, --agent <agent>', 'user agent', 'Remote Keyboard');
    };
